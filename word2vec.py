...
def make_target_context_word_lists(corpus, word2ind, vocab_sz, context_win_sz=2):
    ...
    for sentence in corpus:
        for i in range(len(sentence)):
            context_list = []
            for k in range(-context_win_sz, context_win_sz+1):
                if k != 0:
                    index = i + k
                    if index >=0 and index < len(sentence):
                        context_list.append(index)
            context_words_int.append(np.asarray(context_list))
            target_onehot = np.zeros(vocab_sz)
            target_onehot[word2ind[sentence[i]]] = 1
            target_words_onehot.append(target_onehot)
...
class Skipgram(network.Network):
    ...
    def __init__(self, input_shape=(1, 1016), dense_interior_units=(10,), n_classes=1016,
        ...
        # 3) Dense layer
        self.layers.append( layer.Dense(0, 'dense', units=dense_interior_units[0], n_units_prev_layer=vocab_sz, wt_scale = wt_scale, activation = 'linear', reg = reg))
        # 4) Dense softmax output layer
        self.layers.append( layer.Dense(1, 'dense', units = n_classes, n_units_prev_layer = dense_interior_units[0], wt_scale = wt_scale, activation = 'softmax_embedding', reg = reg))
    ...
    def fit(self, targets_train, contexts_train, n_epochs=10, print_every=100):
        ...
        for e in range(n_epochs):
            ...
            for i in range(iter_per_epoch):
                features_batch = targets_train[i]
                y_batch = contexts_train[i]
                loss += self.forward(np.expand_dims(features_batch, axis = 0), y_batch)
                self.backward(y_batch)
                for layer in self.layers:
                    layer.update_weights()
            self.loss_history.append(loss/iter_per_epoch)
    ...
    def get_word_vector(self, word2ind, word):
        ...
        index = word2ind[word]
        return self.layers[0].wts[index, :]

    def get_all_word_vectors(self, word2ind, wordList, axis = 1):
        ...
        for i in range(len(wordList)):
            word = wordList[i]
            if i == 0:
                all_vector =np.expand_dims(self.get_word_vector(word2ind, word), axis = axis)
            else:
                all_vector = np.concatenate((all_vector, np.expand_dims(self.get_word_vector(word2ind, word), axis = axis)), axis = axis)
        return all_vector.T
