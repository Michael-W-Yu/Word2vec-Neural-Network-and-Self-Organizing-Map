...
class Network():
    ...
    def fit(self, x_train, y_train, x_validate, y_validate, mini_batch_sz=100, n_epochs=10, acc_freq=4):
        ...
        for i in range(n_iter):
            ...
            if i == 0:
                print("Time taken for iteration 0: ", (time.time()-start_time)/60)
        if self.verbose > 0:
            print('Finished training!')

        return self.loss_history.copy(), self.train_acc_history.copy(), self.validation_acc_history.copy()
...
#             if i == 0:
#                 output.append(self.layers[i].forward(inputs))
#             else:
#                 output.append(self.layers[i].forward(output[i-1]))
#         final_net_in = self.layers[-1].net_in
#         return np.argmax(final_net_in, axis = 1)
    ...
    def forward(self, inputs, y):
        ...
        #self.layers[3].forward(self.layers[2].forward(self.layers[1].forward(self.layers[0].forward(inputs))))

        loss = self.layers[-1].loss(y)
        regularization = self.wt_reg_reduce()
    ...
    def wt_reg_reduce(self):
        ...
        for i in (self.wt_layer_inds):
            wt_reg += (1/2) * self.reg * np.sum(np.square(self.layers[i].wts))
    ...
    def backward(self, y):
        ...
        d_b = None
        for i in range (len (self.layers)):
            d_upstream, d_wts, d_b = self.layers[-(i+1)].backward(d_upstream, y)
...
class ConvNet4(Network):
    ...
    def __init__(self, input_shape=(3, 32, 32), n_kers=(32,), ker_sz=(7,), dense_interior_units=(100,),
        ...
        self.layers.append( layer.Conv2D(0, 'conv2D', n_kers[0], ker_sz[0], n_chans=n_chans,wt_scale = wt_scale, activation='relu', reg = reg))
        # 2) 2x2 max pooling layer
        self.layers.append( layer.MaxPooling2D(1, 'pool', pool_size = pooling_sizes[0], strides = pooling_strides[0], activation = 'linear', reg = reg))
        # 3) Dense layer
        self.layers.append( layer.Dense(2, 'dense', units=dense_interior_units[0], n_units_prev_layer=(filter_ops.get_pooling_out_shape(w, pooling_sizes[0], pooling_strides[0])**2 * n_kers[0]), wt_scale = wt_scale,activation = 'relu', reg = reg))
        # 4) Dense softmax output layer
        self.layers.append( layer.Dense(3, 'softmax output layer', units = n_classes, n_units_prev_layer = dense_interior_units[0], wt_scale = wt_scale, activation = 'softmax', reg = reg))
...
class ConvNet4Accel(Network):
    ...
    def __init__(self, input_shape=(3, 32, 32), n_kers=(32,), ker_sz=(7,), dense_interior_units=(100,),
        ...
        # 1) Input convolutional layer
        self.layers.append( accel.Conv2DAccel(0, 'conv2D', n_kers[0], ker_sz[0], n_chans=n_chans,wt_scale = wt_scale, activation='relu', reg = reg))
        # 2) 2x2 max pooling layer
        self.layers.append( accel.MaxPooling2DAccel(1, 'pool', pool_size = pooling_sizes[0], strides = pooling_strides[0], activation = 'linear', reg = reg))
        # 3) Dense layer
        self.layers.append( layer.Dense(2, 'dense', units=dense_interior_units[0], n_units_prev_layer=(filter_ops.get_pooling_out_shape(w, pooling_sizes[0], pooling_strides[0])**2 * n_kers[0]), wt_scale = wt_scale,activation = 'relu', reg = reg))
        # 4) Dense softmax output layer
        self.layers.append( layer.Dense(3, 'softmax output layer', units = n_classes, n_units_prev_layer = dense_interior_units[0], wt_scale = wt_scale, activation = 'softmax', reg = reg))
...
class ConvNet4AccelExt(Network):
    '''
    Makes a ConvNet4 network with the following layers: Conv2D -> MaxPooling2D -> Dense -> Dense
    ...
    def __init__(self, input_shape=(3, 32, 32), n_kers=(32,), ker_sz=(7,), dense_interior_units=(100,),
        ...
        self.layers.append( layer.Dense(3, 'dense', units=dense_interior_units[0], n_units_prev_layer=dense_interior_units[0], wt_scale = wt_scale,activation = 'relu', reg = reg))
        # 4) Dense softmax output layer
        self.layers.append( layer.Dense(4, 'softmax output layer', units = n_classes, n_units_prev_layer = dense_interior_units[0], wt_scale = wt_scale, activation = 'softmax', reg = reg))
        #print("number of layers", len(self.layers))
