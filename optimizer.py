...
# Extension: New optimizer
class Adagrad(Optimizer):
    '''Update weights using Adaptive Gradient Descent (Adagrad) with momentum
    update rule.
    '''
    ...
    def update_weights(self):
        ...
        '''
        self.wts = self.wts - self.lr/(np.sqrt(np.sum(np.square(self.d_wts), axis = 0)))*self.d_wts[-1]
        return self.wts.copy()
