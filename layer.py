...
class Layer():
    ...
    def softmax(self):
        ...
        No return
        '''
        z_net_in_adj = np.exp(self.net_in - np.max(self.net_in, axis = 1, keepdims = True))
        self.net_act = z_net_in_adj/np.sum(z_net_in_adj, axis = 1, keepdims = True)
    ...
    def skipgram(self, y):
        L = y.shape[0]*np.log(np.sum(np.exp(self.net_in), axis = 1))- np.sum(np.take(self.net_in, y))
        return L[0]
    ...
    def cross_entropy(self, y):
        ...
        '''
        loss = -np.sum(np.log(self.net_act[np.arange(self.net_in.shape[0]), y])/self.net_in.shape[0])
        return loss
    ...
    def backward_netAct_to_netIn(self, d_upstream, y):
        ...
        d_net_act = None
        if self.activation == 'relu':
            d_net_act = self.net_act.copy()
            d_net_act[d_net_act > 0] = 1
            d_net_in = d_upstream*d_net_act
            # TODO: compute correct gradient here
        elif self.activation == 'linear':
            d_net_act = np.ones(d_upstream.shape)
            d_net_in = d_upstream*d_net_act
            # TODO: compute correct gradient here
        elif self.activation == 'softmax':
            ...
            d_net_in = y.shape[0]*d_upstream - np.sum(self.one_hot(y,self.wts.shape[1]), axis = 0)
        else:
            raise ValueError('Error! Unknown activation function ', self.activation)
        return d_net_in
...
class Dense(Layer):
    ...
    def __init__(self, number, name, units, n_units_prev_layer,
        ...
        '''
        #print('n_units_prev_layer', n_units_prev_layer)
        #print('units', units)
        self.wts = np.random.normal(loc=0, scale=1, size=(n_units_prev_layer, units))
        self.wts = wt_scale*self.wts
        self.b = np.random.normal(loc=0, scale=1, size=(units,))
        self.b = wt_scale*self.b
    ...
    def compute_net_in(self):
        ...
        '''
        #print('original input shape',self.input.shape)

        # reshaped_input = 1
        # for i in range (self.input.ndim-1):
        #     reshaped_input *= self.input.shape[i+1]
        # reshaped_input = self.input.reshape((self.input.shape[0], reshaped_input))
        #reshaped_input = self.input.reshape((self.input.shape[0], self.input.shape[1]*self.input.shape[2]*self.input.shape[3]))
...
class Conv2D(Layer):
    ...
    def __init__(self, number, name, n_kers, ker_sz,
        ...
        '''
        self.wts = np.random.normal(loc=0, scale=1, size=(n_kers, n_chans, ker_sz, ker_sz))
        self.wts = wt_scale*self.wts
        self.b = np.random.normal(loc=0, scale=1, size=(n_kers,))
        self.b = wt_scale*self.b
            ...
            for d in range(n_chans):
                for y in range(out_y):
                    for x in range(out_x):
                        max_index = np.unravel_index(np.argmax(self.input[img_index, d, y*self.strides:y*self.strides+self.pool_size, x*self.strides:x*self.strides+self.pool_size]), (1,1,self.pool_size,self.pool_size))
                        y_window = max_index[2]
                        x_window = max_index[3]
                        max_x = x_window + x*self.strides
                        max_y = y_window + y*self.strides
