...
#                     min_y = y
#         return (min_x, min_y)
        distance = np.linalg.norm(self.wts - input_vector, axis=2)       
        location = np.unravel_index(np.argmin(distance, axis=None), distance.shape)
        return location
    ...
    def update_wts(self, t, input_vector, bmu_rc):
        ...
        '''
#         param = [0,1]
#         param[0] = self.compute_decayed_param(t, self.init_lr)
#         param[1] = self.compute_decayed_param(t, self.init_sigma)
...
#                 self.wts[x][y] = self.wts[x][y] + self.gaussian(bmu_rc, param[1], param[0])[x][y]*(input_vector-self.wts[x][y])
        ...
        g = self.gaussian(bmu_rc, sigma, lr)
        g = np.expand_dims(g, axis=2)
        self.wts = self.wts + g * (input_vector - self.wts)
    ...
    def init_wts_pca(self,data):
            pd, V, eigenval = pca.pca(data)
            self.wts = pca.init_weight(V,self.wts.shape[0] )
