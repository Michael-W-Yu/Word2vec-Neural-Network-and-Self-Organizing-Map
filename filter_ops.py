...
def conv2_gray(img, kers, verbose=True):
    ...
    pad_x = np.zeros((img_y, padding_x))
    pad_y = np.zeros((padding_y, img_x+2*padding_x))

    img = np.concatenate((np.concatenate((pad_x,img), axis = 1),pad_x), axis = 1)
    img = np.concatenate((np.concatenate((pad_y,img), axis = 0),pad_y), axis = 0)

    if verbose == True:
        print("Image shape should be and is:" ,(img_y+2*padding_y, img_x+2*padding_x), img.shape)
        #print(img)

    for i in range(n_kers):
        kernel = np.flip(np.flip(kers[i],0),1)
        for j in range(img_y):
            for k in range(img_x):
                filteredImg[i,j,k] = np.sum(img[j:j+ker_y, k:k+ker_x]*kernel)
...
def conv2(img, kers, verbose=True):
    ...
    pad_x = np.zeros((n_chans, img_y, padding_x))
    pad_y = np.zeros((n_chans, padding_y, img_x+2*padding_x))

    img = np.concatenate((np.concatenate((pad_x,img), axis = 2),pad_x), axis = 2)
    img = np.concatenate((np.concatenate((pad_y,img), axis = 1),pad_y), axis = 1)

    if verbose == True:
        print("Image shape should be and is:" ,(n_chans, img_y+2*padding_y, img_x+2*padding_x), img.shape)

    for i in range(n_kers):
        kernel = np.flip(np.flip(kers[i],0),1)
        for j in range(img_y):
            for k in range(img_x):
                filteredImg[i,0:n_chans,j,k] = np.sum(img[0:n_chans, j:j+ker_y, k:k+ker_x]*kernel, axis = (1,2))
...
def conv2nn(imgs, kers, bias, verbose=True):
    ...
    pad_x = np.zeros((batch_sz, n_chans, img_y, padding_x))
    pad_y = np.zeros((batch_sz, n_chans, padding_y, img_x+2*padding_x))


    imgs = np.concatenate((np.concatenate((pad_x,imgs), axis = 3),pad_x), axis = 3)
    imgs = np.concatenate((np.concatenate((pad_y,imgs), axis = 2),pad_y), axis = 2)

    if verbose == True:
        print("Image shape should be and is:" ,(batch_sz, n_chans, img_y+2*padding_y, img_x+2*padding_x), imgs.shape)
    for img_index in range(batch_sz):
        img = imgs[img_index]
        for i in range(n_kers):
            kernel = np.flip(np.flip(kers[i],1),2)
            for j in range(img_y):
                for k in range(img_x):
                    filteredImg[img_index,i,j,k] = np.sum(img[0:n_chans, j:j+ker_y, k:k+ker_x]*kernel, axis = (0,1,2)) + bias[i]
...
def get_pooling_out_shape(img_dim, pool_size, strides):
    ...
    int. The size in pixels of the output of the image after max pooling is applied, in the dimension
        img_dim.
    '''
    return int(math.floor((img_dim-pool_size)/strides) + 1)
...
def max_pool(inputs, pool_size=2, strides=1, verbose=True):
    ...
    output = np.zeros((out_y, out_x))


    for y in range(out_y):
        for x in range(out_x):
            output[y,x] = np.max(inputs[y*strides:y*strides+pool_size, x*strides:x*strides+pool_size])
...
def max_poolnn(inputs, pool_size=2, strides=1, verbose=True):
    ...
    out_x = get_pooling_out_shape(img_x, pool_size, strides)
    out_y = get_pooling_out_shape(img_y, pool_size, strides)
    output = np.zeros((mini_batch_sz, n_chans, out_y, out_x))

    for img_index in range(mini_batch_sz):
        for d in range(n_chans):
            for y in range(out_y):
                for x in range(out_x):
                    output[img_index, d, y,x] = np.max(inputs[img_index, d, y*strides:y*strides+pool_size, x*strides:x*strides+pool_size])
